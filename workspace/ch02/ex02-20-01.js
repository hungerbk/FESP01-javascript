...
// map() 남은 할일 목록
var reaminList = todolist.map((elem) => (elem.done ? undefined : elem.title)).filter((val) => val !== undefined);
console.log("map() 남은 할일 목록", reaminList);

// reduce() 남은 할일 수
var reaminCount = todolist.reduce((acc, cur) => (!cur.done ? acc + 1 : acc), 0);
...
var hasTodo = todolist.some((elem) => elem.done === false);
console.log("some() 남은 할일이 하나라도 있는가?", hasTodo);

// every() 할일이 모두 완료 되었는가?
var busy = todolist.every((elem) => elem.done === true);
